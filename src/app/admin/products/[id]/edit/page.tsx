      ...
      try {
        ...
        if (productData.product) {
          ...
          if (p.variants && p.variants.length > 0) {
            setHasVariants(true);
            setVariants(
              p.variants.map((v: { id: string; name: string; sku?: string; price: number; originalPrice?: number | null; stockQuantity: number; lowStockThreshold: number; imageUrl?: string | null }) => ({
                id: v.id,
                name: v.name,
                sku: v.sku || '',
        ...
        <div className="p-5 bg-white border border-slate-100 rounded-2xl flex flex-col gap-4 shadow-sm">
          ...
          {hasVariants && (
            <div className="flex flex-col gap-4 border-t border-slate-100 pt-4 mt-2">
              ...
              {variants.length === 0 ? (
                <div className="py-6 text-center text-xs font-medium text-slate-450 italic">
