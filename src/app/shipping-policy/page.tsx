...
export default async function ShippingPolicyPage() {
  const page = await getCmsPage('shipping-policy');
  const title = page?.title || 'Shipping & Delivery Policy';
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main>
        {/* Hero */}
        <div className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900 py-20 px-4 relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto relative z-10">
            ...
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center border border-white/20">
                <Truck className="text-emerald-300" size={28} />
              </div>
              <div>
                <h1 className="text-3xl md:text-4xl font-black text-white mb-2">{title}</h1>
                <p className="text-emerald-200 text-base max-w-2xl">{subtitle}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              {sections.map((section, idx) => (
                <div key={section.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 hover:shadow-md transition-shadow">
                  <div className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 font-bold text-sm flex items-center justify-center flex-shrink-0 mt-0.5">
                      {idx + 1}
                    </div>
