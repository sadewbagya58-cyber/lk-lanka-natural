...
import { getProductById, getCategoryById } from '@/data';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        ...
        <h1 className="text-2xl sm:text-3.5xl font-black text-slate-900 mb-8">Your Shopping Cart</h1>
        ...
        {cartItems.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative w-20 h-20 rounded-xl bg-slate-50 border border-slate-150 overflow-hidden flex items-center justify-center shrink-0">
                        <div className={`absolute inset-0 bg-gradient-to-tr ${product.gradient} opacity-10`} />
                        <ProductIllustration type={product.visualSeed} className="w-10 h-10 text-slate-700/60" />
                      </div>
                        ...
                        <div className="flex flex-col justify-between">
                          <div>
                            ...
                            <Link href={`/products/${product.slug}`}>
                              <h3 className="text-sm font-bold text-slate-800 hover:text-emerald-600 focus:text-emerald-600 transition-colors mt-0.5 leading-snug">
                                {product.name}
                              </h3>
                            </Link>
                            {variantName && (
                              <span className="text-[10px] text-slate-500 font-bold bg-slate-50 px-2 py-0.5 rounded border border-slate-150 mt-1.5 inline-block">
                                Option: {variantName}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 mt-2">
                            <span className={`w-1.5 h-1.5 rounded-full ${product.inventory.inStock ? 'bg-emerald-600' : 'bg-slate-300'}`} />
              ...
              {/* Keep Shopping Trigger */}
              <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-100 shadow-sm mt-4">
                <Link
                  href="/products"
                  className="text-xs font-bold text-emerald-600 hover:text-emerald-700 focus:outline-none flex items-center gap-1.5"
                >
                  ← Continue Shopping
                </Link>
                <button
            ...
            <div className="flex flex-col gap-6">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col gap-4">
                ...
                <button
                  className="h-12 w-full rounded-xl bg-emerald-600 hover:bg-emerald-750 text-white font-bold text-sm flex items-center justify-center gap-2 transition-all mt-4 focus:outline-none shadow-md hover:shadow-lg shadow-emerald-600/10"
                  onClick={() => alert('Proceeding to checkout... (Hostinger MySQL database integration ready)')}
                >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-light mt-0.5 leading-relaxed">
                      Your transaction details are encrypted using banking-grade security protocols.
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-light mt-0.5 leading-relaxed">
                      Products are direct, genuine, organic, and ethically sourced.
                    </p>
                  </div>
                </div>
              </div>
