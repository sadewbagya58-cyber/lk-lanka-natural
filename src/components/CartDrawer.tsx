          ...
          <div className="p-4 bg-emerald-50/40 border-b border-emerald-100/50 flex flex-col gap-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-slate-700 flex items-center gap-1.5">
                ...
                {isFreeShipping ? 'Congratulations!' : `Add ${formatPrice(shippingThreshold - subtotal)} more for free shipping`}
              </span>
              <span className="text-emerald-700 font-black">{isFreeShipping ? 'FREE SHIPPING ACTIVE' : `${Math.round(shippingProgress)}%`}</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          {cartItems.length > 0 ? (
            cartItems.map((item) => {
              ...
              // Extract custom price for selected variants if applicable
              ...
              return (
                ...
                >
                  ...
                  <div className="relative w-16 h-16 rounded-xl bg-white border border-slate-150 overflow-hidden flex items-center justify-center shrink-0">
                    ...
                    <ProductIllustration type={product.visualSeed} className="w-9 h-9 text-slate-700/60" />
                  </div>
                  ...
                  <div className="flex-1 flex flex-col justify-between min-w-0 pr-6">
                    <div>
                      ...
                      {variantName && (
                        <span className="text-[10px] text-slate-400 font-bold bg-white px-2 py-0.5 rounded border border-slate-150 mt-1 inline-block">
                          Option: {variantName}
                        </span>
                      )}
                    </div>
