...
export default function Navbar() {
  ...
  // Zustand dynamic store selectors
  ...
  // Data-driven categories from centralized data
  const dataCategories = getAllCategories();
  const categoryNames = ['All Categories', ...dataCategories.map((c) => c.name)];

  return (
    <>
      <header className="w-full bg-white shadow-sm sticky top-0 z-50 border-b border-slate-100 backdrop-blur-md bg-white/95">
        {/* TOP BAR: Info & Utility */}
        ...
        {/* MAIN HEADER: Search, Logo, Actions */}
        <div className="w-full py-3.5 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
            ...
            {/* Central Large Search Bar (Desktop) */}
            ...
            {/* Utility actions: User Account, Wishlist, Cart */}
            <div className="flex items-center gap-3.5 shrink-0">
              
              {/* User profile — dynamic based on auth state */}
              {!authLoading && user ? (
                <Link href="/account" className="flex items-center gap-2 text-slate-750 hover:text-emerald-650 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500/20 p-1.5 rounded-xl" aria-label="My Account">
                  ...
                  <div className="hidden lg:flex flex-col text-left">
                    ...
                    <span className="text-xs font-black text-slate-800 leading-none truncate max-w-[80px]">
                      {user.name || 'Profile'}
                    </span>
                  </div>
                </Link>
            ...
            >
              <div className="flex flex-col gap-6">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                  ...
                  <button
                    onClick={() => setIsOpen(false)}
                    className="p-1.5 text-slate-400 hover:text-slate-900 hover:bg-slate-50 rounded-lg focus:outline-none"
